...
union(){
    rotate_extrude(convexity=7, $fn=720)
        difference() {
    ...
    difference(){
        cylinder(d=cap_id+cap_thick,h=cap_thick, $fn=90); 
        translate([0,0,-F])
            linear_extrude(height=1+F,convexity=25)
